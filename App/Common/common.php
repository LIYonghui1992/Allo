...
function sendmail($tomail, $subject, $body, $config = '') {
	...
	// GMAIL用户名,必须以@gmail结尾

	$mail -> Password = $config['mail_password'];
	// GMAIL密码

	//$mail->From ="App@163.com";

	//$mail->FromName = "App企业建站系统";
	...
	//$mail->AddReplyTo('147613338@qq.com', 'App'); //回复到这个邮箱
		...
		foreach ($list as $key => $r) {
			...
			if ($r['groupid'] == 6) {
				...
				if (APP_LANG)
					...
					if ($r['lang']) {$temp[$r['lang']][$r['varname']] = $r['value'];
			...
			foreach ((array)$lang as $key => $r) {
				...
				if ($langid) {
					...
					foreach ($list as $key => $val) {
						...
						$data[$val[$pkid]] = $val;
						$product_group = M('Product_group') -> where('id=' .$val['product_group_id'])->find();
						$product_model = M('Product_model') -> where('id=' .$product_group['product_model_id'])->find();
						$data[$val[$pkid]]['product_model'] = $product_model['title'];
					}
//error_log('sssss'. $lang.','.count($data));
					F('Product_' . $lang, $data);
			...
			foreach ((array)$lang as $key => $r) {
				...
				if ($langid) {
					...
					foreach ($list as $key => $val) {
						...
						$data[$val[$pkid]] = $val;
						$product_model = M('Product_model') -> where('id=' .$val['product_model_id'])->find();
						$data[$val[$pkid]]['product_model'] = $product_model['title'];

					}
					
					F('Product_group_' . $lang, $data);
		...
		if (APP_LANG) {
			...
			foreach ((array)$lang as $key => $r) {
				...
				if ($langid) 
				{
					...
					$list = $Model -> order('listorder') -> select();// where('lang=' . $langid) 
...
function thumb($f, $tw = 300, $th = 300, $autocat = 0, $nopic = 'nopic.jpg', $t = '') {
	...
	if ($tmp == 'gif' || $tmp == 'png') {
		...
		$background_color = imagecolorallocate($timg, 0, 255, 0);
		//  指派一个绿色

		imagecolortransparent($timg, $background_color);
...
function Pinyin($_String) {
	...
	for ($i = 0; $i < strlen($_String); $i++) {
		...
		if ($_P > 160) { $_Q = ord(substr($_String, ++$i, 1));
			$_P = $_P * 256 + $_Q - 65536;
		}

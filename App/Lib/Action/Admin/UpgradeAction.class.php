...
class UpgradeAction extends AdminbaseAction {
	...
	private $_filearr = array('App', 'Public', 'Core', '');

	private $_url = 'http://download.App.cn/';
		...
		if(!empty($_GET['do'])) {
			...
			foreach($patchlist as $k=>$v) {		 
				...
				if($this->copyfileerror){
					...
					if(file_exists($upgradezip_source_path.'/App.sql')){

						$sqldata = file_get_contents($upgradezip_source_path.'/App.sql');
	...
	public function checkfile() {
			...
			$App_filemd5 = @file_get_contents($this->_url.$var.'/md5file/'.UPDATETIME.".php");

			$App_filemd5_arr = json_decode($App_filemd5, 1);unset($App_filemd5);



			$filemd5_arr_key = array_keys($this->filemd5_arr);

			$App_filemd5_arr_key = array_keys($App_filemd5_arr);



			$editfiles = array_diff($App_filemd5_arr, $this->filemd5_arr);

			$lostfile = array_diff( $App_filemd5_arr_key,$filemd5_arr_key);

			$unknowfile = array_diff($filemd5_arr_key, $App_filemd5_arr_key);

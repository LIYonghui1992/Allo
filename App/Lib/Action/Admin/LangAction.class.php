...
class LangAction extends AdminbaseAction {
	...
	function insert() {
		...
		$r =dir_copy(LANG_PATH.'zh/',$lang_path); 
		...
		if ($id !==false) {
			...
			$db->execute("INSERT INTO `App_config`  (`varname`,`info`,`groupid`,`value`,`lang`) VALUES ('site_name','网站名称','2','','".$id."'),

			('site_url','网站网址','2','','".$id."'),

			('logo','网站LOGO','2','./Public/Images/logo.gif','".$id."'),

			('site_email','站点邮箱','2','admin@App.cn','".$id."'),
...

	{
		...
		foreach($_POST as $key=>$r){
			...
			if($r)
				...
				$data[strtoupper($key)]=$r;
			
			else
			
				$data[strtoupper($key)]=strtoupper($key);

		}
		
		F($file,NULL, $this->langpath);
        
		F($file,$data, $this->langpath); 
		
		$r = F($file,'', $this->langpath);

...
class PricelistAction extends AdminbaseAction {
   ...
   */
   	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15)
   	{
  		...
  		if(!empty($keyword) && !empty($searchtype))
  		{
  			$sqlCond .= ' and a.title like \'%'.$keyword.'%\' ';
  		}
  		...
  		if ($count > 0) {
  			...
  			//$field= (empty($field) || $field=='*') ? '*' : 'id,catid,url,posid,a.title,thumb,title_style,userid,username,hits,createtime,updatetime,status,listorder' ;
  			$field=  'a.*,b.product_group_id,b.product_model_id' ;
  			...
  			//模板赋值显示
  			//var_dump(json_encode($voList[0]['product_group_id']));
   ...
   function index(){ 
		...
		foreach($product as $k=>$v){
			...
			$color_list[$v['id']] = $v['color'];
			
		}
		
		$product_groups = D('product_group')-> select();
		foreach($product_groups as $k=>$v){			
			//$product_group_list[$v['id']] = $v['title'];
			$product_group_list[$v['id']] = $v;
			
		}
		
		$product_models = D('product_model')-> select();
		foreach($product_models as $k=>$v){			
			//$product_group_list[$v['id']] = $v['title'];
   ...
   */
   function bk_params(){
    	...
    	$where['id'] = $price_id; 
    	$where2['price_id'] = $price_id;    
    	$param_list = M('product_param') -> where($where2)->order('type asc,listorder desc')->select();
    	     	
    	$price = M('Pricelist') -> where($where)->find();    
   ...
   function bk_save_params(){
    	...
    	if (IS_POST) {
    		$price_id = $_POST['price_id'];
  	 	  ...
  	 	  if(empty($price_id) || empty($value) || empty($title)  ){
          $rs = array('status' => 3,'msg'=>'miss param');
  	 	  ...
  	 	  $data = array('param' => $title,  'value' => $value, 			  
 		                  'price_id'=>$price_id, 'type'=>$type,'lang'=>LANG_ID,

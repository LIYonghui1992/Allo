...
class AttachmentAction extends  Action {
		...
		if(!$this->userid){
			...
			if($this->isadmin){
				...
				if(cookie('auth')){
					...
					$App_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$HTTP_USER_AGENT);
	...
	public function index(){
		...
		$enupsetup = authcode($auth, 'DECODE', $App_auth_key);
    ...
    $upload->allowExts = explode(',', 'jpg,jpeg,gif,png,pdf,doc');  
        ...
        if (!$upload->upload()) { 
			...
			$data['filepath'] = '/Uploads/'.strtolower($uploadList[0]['savename']);

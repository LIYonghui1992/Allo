...
class AdminbaseAction extends Action
{
	...
	function _initialize()
	{
		...
		$this->cache_model=array('Lang','Menu','Config','Module','Role','Category','Posid','Field','Type','Urlrule','Dbsource','Product','Product_group','Country','Design_winner');
...
error_log('adminbase');		
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'', './App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
			L(include LANG_PATH.$this->sysConfig['DEFAULT_LANG'].'/common.php');
			$T = F('config_'.$this->sysConfig['DEFAULT_LANG'],'', './App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}
		$this->assign('T',$T);
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,'./App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,'./App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			if($cat['listtype']==1){
				...
				if($count){
					...
					$field =  $field ? $field : 'id,catid,userid,category,url,username,title,title_style,keywords,description,thumb,createtime,hits';
			...
			$r=$this->buildHtml($filename,$dir,'./App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index($sitemap)
    {
		...
		//print_r(__ROOT__);exit;
 		//$r=$this->buildHtml('test','./','Home@Index/Index_index','utf8');exit;
		if(empty($sitemap)){
			$this->assign('ishome','home');
			if(!$this->sysConfig['HOME_ISHTML']) $this->error(L('NO_HOME_ISHTML'));
			$this->assign('bcid',0);
			/*$r=$this->buildHtml('index','./'.$lang,'./App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));*/
			$r=$this->buildHtml('index','./'.$lang,'Home@Index/Index_index');
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHtml('sitemap','./'.$lang,'./App/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }

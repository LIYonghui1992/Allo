...
class BaseAction extends Action
{
    ...
    public function _initialize() {
			...
			if(APP_LANG){
				...
				//if( empty($_COOKIE('allocacoc_think_second')) && empty($_GET['l'])){ $_GET['l'] = 'en';}
				if(empty( $_COOKIE['allocacoc_think_second']) && empty($_GET['l'])){ $_GET['l'] = 'en';}
				...
				if(get_safe_replace($_GET['l'])){
					...
					$lang=$_COOKIE['allocacoc_think_language'];
			...
			if(empty($_COOKIE['allocacoc_country_id'])){
				
				//国家
				
				/*$_COOKIE['allocacoc_country_id'] = 7;
				
				cookie('country_id',7);*/
				
				$_COOKIE['allocacoc_country_id'] = 26;
				
				cookie('country_id',26);
				
			}

			$this->assign('country_id',$_COOKIE['allocacoc_country_id']);
			...
			if(cookie('auth')){
				...
				$App_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$_SERVER['HTTP_USER_AGENT']);



				list($userid,$groupid, $password) = explode("-", authcode(cookie('auth'), 'DECODE', $App_auth_key));
			...
			$country = M('country') -> where('status=1') -> order('title asc') -> select();

			

			foreach($country as $k=>$v){

				$country[$k]['lang'] = M('lang') -> where('id='.$v['lang']) -> find();
			...
			$web_config2 = M('config') -> select();
			
			
			$Facebook_link = $web_config2[5]['value'];
			$Linkedin_link = $web_config2[67]['value'];
			$Instagram_link = $web_config2[68]['value'];
		   
			$this->assign('Facebook_link',$Facebook_link);
			$this->assign('Linkedin_link',$Linkedin_link);
			$this->assign('Instagram_link',$Instagram_link);
			...
			$sitetitle_where['varname'] = array('like','%seo_title%');
			
			$sitetitleconfig = M('config')->where($sitetitle_where)->find();
			
			$this->assign('seo_title',$sitetitleconfig['value']);
			
			
			$sitekeywords_where['varname'] = array('like','%seo_keywords%');
			
			$sitekeywordsconfig = M('config')->where($sitekeywords_where)->find();
			
			$this->assign('seo_keywords',$sitekeywordsconfig['value']);
			
			
			$sitedescription_where['varname'] = array('like','%seo_description%');
			
			$sitedescriptionconfig = M('config')->where($sitedescription_where)->find();
			
			$this->assign('seo_description',$sitedescriptionconfig['value']);
  ...
  public function verify_big()
  {
    ...
    import("@.ORG.Image");
    ob_clean();
    Image::buildImageVerify_big(4,2,$type,160,50,'comment_verify');//$_SESSION['comment_verify']

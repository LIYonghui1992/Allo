...
class DesignNestAction extends BaseAction {	
    ...
    */
	...
	public function index() {
		...
		$where['status'] = $design_challenge['id'];
		$where['challenge_id'] = 1;
		$promotion_list = M('design_promotion') -> where($map) -> select();
		...
		foreach ($fund_list as $k => $v) {
			...
			$fund_list[$k]['target_money'] = intval($v['funding_goal'])*substr ($v['price'],1);
		...
		$suc_fund_list = M('design_winner') -> where($where2)->where("funding_goal <= fake_order_qty") -> order('listorder asc,updatetime desc')->limit(0,4)->select();
		foreach ($suc_fund_list as $k => $v) {
			...
			$suc_fund_list[$k]['target_money'] = intval($v['funding_goal'])*substr ($v['price'],1);
		}
	...
	private function search_current_project($suc,$keyword,&$count,$start,$size){
		...
		if($suc==0)
		  ...
		  $count = M('design_winner') -> where($where) ->where("funding_goal <= fake_order_qty") ->count();
		...
		if($suc==0)
		  ...
		  $fund_list = M('design_winner') -> where($where) ->where("funding_goal <= fake_order_qty")-> limit($start.','.$size)->order($sort)->select();
	...
	public function project_search_new(){
		...
		//$where['lang'] = LANG_ID;
		$where['status'] = 1;			
		$where['title|developer']=array('exp',"like '%".$_GET['keyword']."%'");		
		$sort = 'listorder asc,updatetime desc';

		$count = M('design_winner') -> where($where) ->count();
		$fund_list = M('design_winner') -> where($where) -> limit(0,16)->order($sort)->select();
	...
	*/
	...
	public function ajax_search_projectlist_new(){
		//error_log(json_encode($_POST));
		if (IS_POST){
  		...
  		$fund_list = M('design_winner') -> where($where) -> limit($curr_count,8)->order($sort)->select();
  ...
  public function pagelist() {
	...
	*/
    ...
    if (!empty($fund['slide_pic'])) {
		...
		if (!empty($fund['banner'])) {
			
			$banner_s = explode(':::', $fund['banner']);
			
			foreach ($banner_s as $k => $v) {

				$banner_s[$k] = strstr($v, '|', TRUE);
...
//		$fund['winner_text'] = $fund['winner_text'];
		...
		$fund['designer_introd']=stripslashes($fund['designer_introd']);
		//feature list
...
//			exit;
  ...
  */
        ...
        */
		...
		$mail->SMTPAuth = true; // turn on SMTP authentication
		$mail->Username = "designnest.orders@allocacoc.com";//"designhouse.orders@allocacoc.com"; // SMTP username 注意：普通邮件认证不需要加 @域名
		$mail->Password = "j~9B96sw";//"egx-7VW-Y4o-Bps"; // SMTP password
...
';
   ...
   public function submit_designidea_bk_20160927()
   {
   	 if (IS_POST){  	 	
   	 	 $project_title = $_POST['project_title'];
   	 	 $firstname = $_POST['firstname'];
  ...
  */
  ...
  public function disclaimer() {
	...
	/**
  ** backup, Leo update the function
  */
  public function detail_20161021() {
  	/*
  		//头部banner
		//$map['lang'] = LANG_ID;		
		$map['lang'] = 9;
    ...
    * backup,Leo update the function
    */
    public function order_designwinner_20161021()
    {
    	//头部banner
  		//$map['lang'] = LANG_ID;		
  		$map['lang'] = 9;
...
/**
    ...
    * backup, Leo update the function
    */
		public function submit_order_20161021() {
	...
	** backup.Leo update the function
	*/
	public function shipPrice_20161021(){

...
class Form extends Think {
	...
	public function editor($info,$value){
		...
		if(ACTION_NAME=='add'){
		 ...
		 $alowuploadexts =  'jpg,gif,png,pdf';
		...
		if(!isset($Config[attach_allowext]))
		$Config[attach_allowext]='jpg,jpeg,gif,png';
	...
	public function language_id($info,$value){

        $newinfo = $info;

        $language_ids=F('Language');
		
        $options=array();

        $options[0]= L('please_chose');

        foreach($language_ids as $key=>$r) {
	...
	public function country_id($info,$value){

        $newinfo = $info;

        $language_ids=F('Country_'.LANG_NAME);
	...
	public function product_group_id($info,$value){

        $newinfo = $info;

        $product_group_ids=F('Product_group_'.LANG_NAME);
		
        $options=array();

        $options[0]= L('please_choose');

        foreach($product_group_ids as $key=>$r) {//error_log(json_encode($r));

           
           if(empty($r['product_model']))
             $options[$key]=$r['title'];
           else
             $options[$key]=$r['title'].'--Type '.$r['product_model'];
	...
	public function product_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Product_'.LANG_NAME);
		
        $options=array();

        $options[0]= L('please_chose');

        foreach($product_model_ids as $key=>$r) {

           $options[$key]=$r['title'].'--Type '.$r['product_model'].'--'.$r['color'];
	...
	public function product_model_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Product_model_'.LANG_NAME);
	...
	public function version_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Version_'.LANG_NAME);
	...
	public function design_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Design_'.LANG_NAME);
	...
	public function designyear_id($info,$value){
		

        $newinfo = $info;

        $product_model_ids=F('Design_years_'.LANG_NAME);
    ...
    public function office_id($info,$value)
	  { 
		    ...
		    $result = M('Office')->field('id,title')->select();
		    $options=array();
		    $options[0]= L('please_choose');
	  ...
	  public function designwinner_id($info,$value)
	  { 
	  	/**
        $newinfo = $info;

        $product_model_ids=F('Design_winner_'.LANG_NAME);
        ...
        foreach($product_model_ids as $key=>$r) {
		    ...
		    */
		    ...
		    if(!isset($this->data['lang']))
		    $this->data['lang'] = 9;
		    //$result = M('Design_winner')->field('id,title')->where(array('lang'=>array('eq',$this->data['lang'])))->select();
		    $result = M('Design_winner')->field('id,title')->select();
		    $options=array();
		    $options[0]= L('please_choose');
		    foreach($result as $key=>$r) {
    ...
    public function shipfee_set_id($info,$value)
	  { 
		    $newinfo = $info;
		    /*
		    if(!isset($this->data['lang']))
		    ...
		    $result = M('Design_winner')->field('id,title')->where(array('lang'=>array('eq',$this->data['lang'])))->select();
		    */
		    $result = M('ShipPrice_sets')->field('id,title')->select();
		    $options=array();
		    $options[0]= L('please_choose');
		    foreach($result as $key=>$r) {
	...
	public function download_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Download_'.LANG_NAME);
	...
	public function downloadt_id($info,$value){

        $newinfo = $info;

        $product_model_ids=F('Download_two_'.LANG_NAME);
